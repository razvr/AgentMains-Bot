...
const stubChaosBot = (chaosBot) => {
  ...
  chaosBot.testCmdMessage = (message) => {
    chaosBot.logger.warn("#testCmdMessage is deprecated. Please use #testMessage");
  ...
  chaosBot.testMessage = async ({ channel, member, content }) => {
    ...
    await chaosBot.emit('message', buildMessage({ channel, member, content })).toPromise();
    return msgResponse;
  };
...
function testCommand({ pluginName, commandName, message = null, args = {}, flags = {} }) {
  this.logger.warn('testCommand is deprecated. Please use testMessage instead.');
...
function testConfigAction({ pluginName, actionName, message = null, args = {} }) {
  this.logger.warn('testConfigAction is deprecated. Please use testMessage instead.');
  message = buildMessage(message);
