...
const stubChaosBot = (chaosBot) => {
   ...
   * @param msg.author {Object} The user that sent the message
  ...
  chaosBot.createMessage = (msg = {}) => {
    ...
    if (!msg.guild.members) msg.guild.members = new Discord.GuildMemberManager(msg.guild);
    if (!msg.guild.roles) {
      msg.guild.roles = new Discord.RoleManager(msg.guild);
      let everyoneRole = { id: msg.guild.id, name: "everyone" };
      msg.guild.roles.cache.set(everyoneRole.id, everyoneRole);
    }
    ...
    if (!msg.member.roles) msg.member.roles = new Discord.GuildMemberRoleManager(msg.member);
    if (!msg.member._roles) msg.member._roles = [];
  ...
  chaosBot.testMessage = async (message) => {
    ...
    message = chaosBot.createMessage(message);
