const { of, from, throwError, ReplaySubject, Subject, EMPTY } = require('rxjs');
const { flatMap, tap, last, toArray, mapTo, catchError, filter, map } = require('rxjs/operators');
...
class ChaosCore {
  ...
  _addEventListeners() {
    this.addEventListener('guildCreate', (guild) => this.onJoinGuild(guild));
  }
    ...
    Object.values(Discord.Constants.Events).forEach((event) => {
      ...
      this.streams[streamName] = streamSubject.pipe(
        tap(() => this.logger.warn(
          `chaos.streams.${streamName} is deprecated. ` +
          `Please use chaos.addEventListener('${event}', () => {}) instead.`,
        )),
      );

      this.addEventListener(event, (payload) => streamSubject.next(payload));
    });
  }
