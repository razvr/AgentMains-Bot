...
class ChaosCore {
   ...
   * @param config {ChaosConfig} configuration settings for this Nix bot
   */
  constructor(config) {
  ...
  listen() {
    if (!this.listening) {
      ...
      Rx.Observable.of('')
        ...
        .flatMap(() => {
          this.logger.verbose(`Starting onListen hooks`);
          return this.onListen()
            .do(() => this.logger.verbose(`onListen hooks complete`));
        })
        .flatMap(() => {
  ...
  onListen() {
    return Rx.Observable.of('')
      .do(() => this.logger.debug(`Starting onListen hooks`))
      .flatMap(() => this.runHook(this.servicesManager, 'onListen', [], true))
      .flatMap(() => this.runHook(this.pluginManager, 'onListen', [], true))
      .defaultIfEmpty('')
      .last()
      .map(() => true)
      .do(() => this.logger.debug(`Completed onListen hooks`));
  }

  onJoinGuild(guild) {
    return Rx.Observable.of('')
      .do(() => this.logger.debug(`Starting onJoinGuild hooks for guild '${guild.name}' (${guild.id})`))
      .flatMap(() => this.runHook(this.dataManager, 'onJoinGuild', [guild], true))
      .flatMap(() => this._readyDataSource(guild))
      .flatMap(() => this.runHook(this.servicesManager, 'onJoinGuild', [guild], true))
      .flatMap(() => this.runHook(this.pluginManager, 'onJoinGuild', [guild], true))
      .defaultIfEmpty('')
      .last()
      .map(() => true)
      .do(() => this.logger.debug(`Completed onJoinGuild hooks for guild ${guild.id} (${guild.name})`));
  }
  ...
  _readyDataSource(guild) {
    let pluginService = this.getService('core', 'pluginService');

    this.logger.debug(`Preparing default data for guild ${guild.id}`);
    return pluginService.prepareDefaultData(this, guild.id);
  }
  ...
  _initalizeDataSource() {
    return Rx.Observable.of('')
      .flatMap(() => this.dataManager.onListen())
      .flatMap(() =>
        Rx.Observable
