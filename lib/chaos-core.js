...
class ChaosCore {
  ...
  constructor(config) {
    ...
    if (config.reponseStrings) {
      this.logger.warn("Config.responseStrings is deprecated. Please use .strings instead.");
      this.applyStrings(config.reponseStrings);
    } else {
      this.applyStrings(config.strings);
    }
  }

  get responseStrings() {
    this.logger.warn("chaos.responseStrings is deprecated. Please use chaos.strings.");
    return this.strings;
  }
  ...
  get shutdown$() {
    this.logger.warn("chaos.shutdown$ is deprecated. Please use .on('chaos.shutdown', () => {}) instead");
  ...
  listen() {
    if (!this.listening) {
      ...
      this._shutdown$ = this._shutdownSubject.pipe();

      this._shutdown$.pipe(
        tap(() => this.logger.verbose(`Starting chaos.shutdown event`)),
        flatMap(() => this.emit("chaos.shutdown")),
        tap(() => this.logger.verbose(`Completed chaos.shutdown event`)),
  ...
  notifyError(embedFields) {
    this.logger.warn("chaos.notifyError is deprecated. Please use handleError instead");
  ...
  catchError(embedFields) {
    this.logger.warn("chaos.catchError is deprecated. Please use handleError instead");
    return (source) => source.pipe(
      catchError((error) => of('').pipe(
        flatMap(() => this.handleError(error, embedFields)),
        flatMap(() => EMPTY),
