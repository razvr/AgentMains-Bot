const { of, from, throwError, ReplaySubject, EMPTY } = require('rxjs');
...
class ChaosCore {
  ...
  async listen() {
    if (!this.listening) {
      ...
      await this.emit('chaos.startup').toPromise();
