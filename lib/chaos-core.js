...
const ChaosLogger = require("./utility/chaos-logger");
...
class ChaosCore {
  ...
  onListen() {
    return Rx.Observable.of('')
      ...
      .flatMap(() => this.runHook(this.moduleManager, 'onListen', [], true))
      .defaultIfEmpty('')
      .last()
      .map(() => true)
  ...
  onJoinGuild(guild) {
    return Rx.Observable.of('')
      ...
      .flatMap(() => this.runHook(this.moduleManager, 'onJoinGuild', [guild], true))
      .defaultIfEmpty('')
      .last()
      .map(() => true)
  ...
  _startEventStreams() {
    ...
    Object.values(Discord.Constants.Events).forEach((eventType) => {
      ...
      this.logger.debug(`Created stream chaos.streams.${streamName}`);
    });
