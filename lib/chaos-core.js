...
class ChaosCore {
  ...
  _initializeEvents() {
    ...
    this.registerEvent('chaos.servicesReady');
    ...
    this.on('chaos.listen', () => this.onListen());
  }
    ...
    if (!this.listening) {
      ...
      of('').pipe(
        ...
        flatMap(() => of('').pipe(
          ...
          tap(() => this.logger.info(`Data source ready`)),
        )),
        flatMap(() => of('').pipe(
          tap(() => this.logger.verbose(`Starting onListen hooks`)),
          flatMap(() => this.emit('chaos.listen')),
          tap(() => this.logger.info(`Completed onListen hooks`)),
        )),
