...
class PermissionsService extends Service {
  ...
  getPermissionsData(guildId, level) {
    if (this.levels.indexOf(level.toLowerCase()) === -1) {
      return Rx.Observable.throw(new PermissionLevelNotFound(`The level ${level} could not be found.`));
    }

    return this.nix.getGuildData(guildId, this.getDatakey(level))
      .flatMap((savedData) => {
        if (!savedData) {
  ...
  setPermissionsData(guildId, level, data) {
    ...
    return this.nix.setGuildData(guildId, this.getDatakey(level), data);
  }
    ...
    return Rx.Observable.from(command.permissions)
      .flatMap((level) =>
        this.getPermissionsData(guildId, level)
          .catch((error) => {
            if (error instanceof PermissionLevelNotFound) {
