...
class CommandService extends Service {
  ...
  runCommandForMsg(message) {
    ...
    let prefixes = this.getPrefixesForMessage(message);
    let parsedCommand = CommandParser.parse(message, prefixes, this.nix.commandManager);

    let context = new Context(parsedCommand, this.nix);
    let response = new Response(message);

    return Rx.Observable
      .of('')
      .flatMap(() => this.filterCanRunCommand(context))
      .map(() => parsedCommand.run(context, response))
