const defaultResponseStrings = {
  config: {
    ...
    moduleNotFound: ({moduleName, prefix}) =>
      `There is no module "${moduleName}". Use \`${prefix}config --list\` to see a list of all config actions.`,
    actionNotFound: ({ moduleName, actionName, prefix}) =>
      `There is no config action "${moduleName} ${actionName}". Use \`${prefix}config ${moduleName} --list\` ` +
