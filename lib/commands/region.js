...
module.exports = new Command({
  ...
  run: (context) => {
    ...
    if (!foundRegion) { return Rx.Observable.just(responses.regionNotFound(context.args.region)) }

    let newRole = findRole(context.message.guild, foundRegion.role);
    if (!newRole) { return Rx.Observable.just(responses.roleNotFound(foundRegion.role)) }
