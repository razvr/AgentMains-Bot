...
const NixLogger = require("./nix-logger");
...
const defaultResponseStrings = require('./reponse-strings');
...
class NixCore {
  ...
  constructor(config) {
    ...
    this.handleHook = (hookReturnValue) => {
      this.logger.trace("nix#handleHook is deprecated. Please import and use NixCore.utility.toObservable");
      ...
      Rx.Observable.of('')
        ...
        .flatMap(() => {
          ...
          return Rx.Observable.from(this.discord.guilds.array())
            ...
            .defaultIfEmpty('')
            .last()
            .do(() => this.logger.verbose(`onNixJoinGuild hooks complete`));
        })
        .do(() => {
  ...
  onNixListen() {
    return Rx.Observable.of('')
      ...
      .flatMap(() => this.runHook(this.pluginManager, 'onNixListen', [], true))
      .defaultIfEmpty('')
      .last()
      .map(() => true)
  ...
  onNixJoinGuild(guild) {
    return Rx.Observable.of('')
      ...
      .flatMap(() => this.runHook(this.pluginManager, 'onNixJoinGuild', [guild], true))
      .defaultIfEmpty('')
      .last()
      .map(() => true)
