...
class NixCore {
  ...
  listen(ready, error, complete) {
    if (!this.listening) {
      ...
      this.main$ =
        Rx.Observable.of('')
          .do(() => {
            this.logger.verbose(`Injecting dependencies into services and commands`);
          ...
          .flatMap(() => {
            ...
            return this._initalizeDataSoruce();
          })
          .flatMap(() => {
          ...
          .flatMap(() => {
            ...
            return this._doOnNixListenHooks()
  ...
  _initalizeDataSoruce() {
    return Rx.Observable.of('')
      .flatMap(() => this._triggerHook(this.dataManager, 'onNixListen'))
      .flatMap(() =>
        Rx.Observable
  ...
  _doOnNixJoinGuildHooks(guild) {
    this.logger.debug(`Starting onNixJoinGuild hooks for guild '${guild.name}' (${guild.id})`);
    return Rx.Observable.of('')
      .flatMap(() => this._triggerHook(this.dataManager, 'onNixJoinGuild', [guild]))
  ...
  _triggerHook(hookListener, hookName, args=[]) {
    ...
    return Rx.Observable
      ...
      .flatMap((returnValue) => Utilities.handleHook(returnValue));
  }
  ...
  _startEventStreams() {
    ...
    let eventStreams = Rx.Observable
      .merge([
        ...
        this.streams.guildCreate$.flatMap((guild) => this._doOnNixJoinGuildHooks(guild)),
