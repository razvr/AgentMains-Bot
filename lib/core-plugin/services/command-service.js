const { of, merge, iif, throwError, Subject } = require('rxjs');
const {
...
const { EnableCommandError, DisableCommandError } = require("../../errors");
...
class CommandService extends Service {
  ...
  onListen() {
    ...
    this.chaos.streams.postCommand$ = new Subject();
  ...
  runCommandForMsg(message) {
    ...
    return of('').pipe(
      ...
      defaultIfEmpty(''),
      tap(() => this.chaos.streams.postCommand$.next({ context, response })),
  ...
  enableCommand(guildId, commandName) {
    ...
    return this._getEnabledCommands(guildId).pipe(
      flatMap((enabledCmds) =>
  ...
  _getEnabledCommands(guildId) {
    return this.chaos.getGuildData(guildId, DataKeys.ENABLED_COMMANDS).pipe(
      filter(Boolean),
      defaultIfEmpty({}),
    );
  }

  _setEnabledCommands(guildId, enabledCmds) {
    return this.chaos.setGuildData(guildId, DataKeys.ENABLED_COMMANDS, enabledCmds).pipe(
      flatMap(() => this._getEnabledCommands(guildId)),
    );
  }
}
