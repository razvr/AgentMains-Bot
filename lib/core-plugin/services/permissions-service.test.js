...
describe('PermissionsService', function () {
  ...
  describe('#getPermissionsData', function () {
    ...
    context('when the permission level is unknown', function () {
      ...
      it('raises a PermissionLevelNotFound error', async function () {
        try {
          await this.permissionsService.getPermissionsData(this.guild.id, this.level).toPromise();
        } catch (error) {
    ...
    context('when the permission level is known', function () {
      ...
      context('when there is no data for the level', function () {
        beforeEach(async function () {
          await this.chaos.setGuildData(this.guild.id, `core.permissions.${this.level}`, null).toPromise();
        });
        ...
        it('emits a default set of data', async function () {
          let data = await this.permissionsService.getPermissionsData(this.guild.id, this.level).toPromise();
      ...
      context('when previous data exists for the level', function () {
        beforeEach(async function () {
          ...
          await this.chaos.setGuildData(this.guild.id, `core.permissions.${this.level}`, this.data).toPromise();
        });
  ...
  describe('#setPermissionsData', function () {
    ...
    context('when the permission level is unknown', function () {
      ...
      it('raises a PermLevelNotFoundError error', async function () {
        try {
          await this.permissionsService.setPermissionsData(this.guild.id, this.level, this.data).toPromise();
        } catch (error) {
          expect(error).to.be.an.instanceOf(PermissionLevelNotFound);
          expect(error.message).to.eq(`The permission level '${this.level}' could not be found.`);
          return;
    ...
    context('when the permission level is known', function () {
      ...
      it('emits the saved data', async function () {
        let savedData = await this.permissionsService.setPermissionsData(this.guild.id, this.level, this.data).toPromise();
        expect(savedData).to.deep.eq(this.data);
      });
    });
  });
  ...
  describe('#addUser', function () {
    ...
    it('adds the user to the given permission level', async function () {
      await this.permissionsService.addUser(this.guild, this.level, this.user).toPromise();
      let savedData = await this.permissionsService.getPermissionsData(this.guild.id, this.level).toPromise();
