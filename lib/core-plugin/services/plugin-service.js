...
class PluginService extends Service {
  constructor(nix) {
    ...
    this._pluginManager = this.nix.pluginManager;
  ...
  enablePlugin(guildId, pluginName) {
    ...
    return this.isPluginEnabled(guildId, module.name)
      .flatMap((isEnabled) => {
        ...
        return this.nix.getGuildData(guildId, DATAKEYS.ENABLED_MODULES);
      })
      .flatMap((savedData) => {
      ...
      .flatMap((savedData) => this.nix.setGuildData(guildId, DATAKEYS.ENABLED_MODULES, savedData))
  ...
  isPluginEnabled(guildId, pluginName) {
    let module = this.nix.getPlugin(pluginName);
  ...
  prepareDefaultData(nix, guildId) {
    return Rx.Observable
      ...
      .flatMap((defaultData) => {
        return this.nix.getGuildData(guildId, defaultData.keyword)
          .flatMap((savedData) => {
            if (typeof savedData === 'undefined') {
              return this.nix.setGuildData(guildId, defaultData.keyword, defaultData.data);
            }
            else {
