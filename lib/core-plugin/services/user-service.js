...
class UserService extends Service {
  ...
  async findMember(guild, userString) {
    ...
    if (userIdMatches) {
      ...
      user = guild.members.cache.array().find((member) => member.user.id === userID);
    } else {
  ...
  async findUser(userString) {
    ...
    if (userIdMatches) {
      ...
      user = this.chaos.discord.users.cache.find((user) => user.tag === userString);
    }
  ...
  async findUserById(userId) {
    try {
      return await this.chaos.discord.users.fetch(userId);
    } catch (error) {
      if (
  ...
  _findMembersByString(guild, userString) {
    return guild.members.cache.array().filter((member) => {
      return (
