...
class PermissionsService extends Service {
  ...
  async removeUser(guild, level, user) {
    let savedData = this.getPermissionsData(guild.id, level);
  ...
  async hasPermission(context, commandName) {
    ...
    if (command.adminOnly) {
      this.chaos.logger.debug('Command is admin only.');
  ...
  async hasPermissionLevel(guild, member, level) {
    ...
    if (levelData.users.indexOf(member.user.id) !== -1) {
      this.chaos.logger.debug(`User has permission level ${levelData.name}`);
      return true;
    } else if (member.roles.some((r) => levelData.roles.indexOf(r.id) !== -1)) {
      this.chaos.logger.debug(`One of the user's roles has permission level ${levelData.name}`);
      return true;
    } else {
      this.chaos.logger.debug(`User does not have permission level ${levelData.name}`);
      return false;
    }
  }
}
