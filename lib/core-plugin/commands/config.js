...
class ConfigCommand extends Command {
        ...
        switch (error.name) {
          case "PluginNotFoundError":
            return response.send({
              type: 'message',
              content: chaos.strings.config.pluginNotFound({
                pluginName,
                prefix,
              }),
            });
          case "PluginNotEnabledError":
            return response.send({
              type: 'message',
              content: chaos.strings.config.pluginNotEnabled({
                pluginName,
                prefix,
              }),
            });
          case "ConfigActionNotFoundError":
            return response.send({
              type: 'message',
              content: chaos.strings.config.actionNotFound({
                pluginName,
                actionName,
                prefix,
              }),
  ...
  runAction(context, response) {
    ...
    return zip(
      ...
      flatMap(([plugin, pluginEnabled]) => {
        if (!pluginEnabled) {
          let error = new PluginNotEnabledError(`The plugin ${plugin.name} is not enabled.`);
  ...
  listActions(context, response) {
    ...
    if (pluginName) {
      response.content = this.chaos.strings.config.actionList({ pluginName });
      response.embed = this.actionListEmbed(context);
    } else {
      response.content = this.chaos.strings.config.pluginList({});
