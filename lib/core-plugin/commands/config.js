...
class ConfigCommand extends Command {
  ...
  async listActions(context, response) {
    ...
    if (context.args.plugin) {
      let plugin = this.chaos.getPlugin(context.args.plugin);
      await response.send({
        content: this.strings.actionList({ pluginName: plugin.name }),
        embed: this.actionListEmbed(prefix, plugin),
      });
    } else {
      await response.send({
        content: this.strings.pluginList({}),
        embed: this.pluginListEmbed(prefix),
      });
    }
  }

  pluginListEmbed(prefix) {
  ...
  actionListEmbed(prefix, plugin) {
    const embed = new Discord.MessageEmbed();

    const configActions = Object.values(this.chaos.configManager.actions)
      .filter((action) => action.pluginName.toLowerCase() === plugin.name.toLowerCase());
