...
module.exports = {
  ...
  runAction(context, response) {
    ...
    return Rx.Observable
      .of('')
      .flatMap(() => {
        this.nix.getModule(moduleName); //Check that the module exists
        let action = this.nix.getConfigAction(moduleName, actionName);

        action.inputs.forEach((input, index) => {
          context.args[input.name] = context.args[`input${index + 1}`];
        });
  ...
  actionListEmbed(context) {
    ...
    let module = context.nix.getModule(moduleName);
