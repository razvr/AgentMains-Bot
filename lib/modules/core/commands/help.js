...
module.exports = {
  ...
  run (context, response) {
    ...
    return Rx.Observable
      ...
      .flatMap((command) => context.nix.commandManager.filterCommandEnabled(context.guild.id, command.name).map(() => command))
      .flatMap((command) => context.nix.permissionsManager.filterHasPermission(context, command.name).map(() => command))
      .toArray()
