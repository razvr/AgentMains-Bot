...
const { flatMap, toArray, tap } = require('rxjs/operators');
    ...
    context('when plugins have a onListen hook', function () {
      ...
      it('triggers the hook for each', function (done) {
        this.chaos.emit('chaos.listen').pipe(
          toArray(),
          tap(() => expect(this.testPlugin1.onListen).to.have.been.calledOnce),
          tap(() => expect(this.testPlugin2.onListen).to.have.been.calledOnce),
          tap(() => expect(this.testPlugin3.onListen).to.have.been.calledOnce),
