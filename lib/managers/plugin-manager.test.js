...
describe('PluginManager', function () {
  ...
  describe("#addPlugin", function () {
    ...
    context('when the plugin has dependencies', function () {
      it('loads the dependencies from npm', function () {
        ...
        expect(this.pluginManager.getPlugin("dependant").name).to.eq("dependant");
      });
    });
    ...
    context('when the plugin has new permission levels', function () {
      beforeEach(function () {
        this.testPlugin.permissionLevels = [
          "test1",
          "test2",
        ];
      });
      ...
      it('adds all permission levels to chaos', function () {
        ...
        expect(this.chaos.addPermissionLevel).to.have.been
          .calledWith(this.testPlugin.permissionLevels[0]);
        expect(this.chaos.addPermissionLevel).to.have.been
          .calledWith(this.testPlugin.permissionLevels[1]);
      });
    });
  });
  ...
  describe("event: guildCreate", function () {
    ...
    it('prepares the default data for reach plugin', async function () {
      await this.chaos.emit('guildCreate', this.guild).toPromise();
      const savedData = await Promise.all([
        this.chaos.getGuildData(this.guild.id, 'data_1').toPromise(),
        this.chaos.getGuildData(this.guild.id, 'data_2').toPromise(),
        this.chaos.getGuildData(this.guild.id, 'data_3').toPromise(),
      ]);
      expect(savedData).to.deep.equal(["value_1", "value_2", "value_3"]);
    });
  });
});
