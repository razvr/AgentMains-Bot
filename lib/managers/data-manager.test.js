...
describe('DataManager', function () {
  ...
  describe('Event: chaos.listen', function () {
    context('when the datasource does not have a onListen', function () {
      ...
      it("does not raise any errors", function (done) {
    ...
    context('when the datasource has a onListen', function () {
      ...
      it("calls the datasource's onListen", function (done) {
        this.chaos.emit('chaos.listen').pipe(
          toArray(),
          tap(() => expect(this.dataManager._dataSource.onListen).to.have.been.called),
        ).subscribe(() => done(), (error) => done(error));
      });
    });
  });
