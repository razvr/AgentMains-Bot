...
const { filter, tap, flatMap } = require('rxjs/operators');
...
class DataManager extends ChaosManager {
  constructor(chaos) {
    ...
    this.chaos.on("chaos.listen:before", () => of('').pipe(
      filter(() => this._dataSource.onListen),
      tap(() => this.logger.warn(
        'Your data source uses the deprecated onListen hook. ' +
        'Please check for an update, or let the developer know about the ' +
        'deprecation. The recommended replacement is ' +
        '`chaos.on("chaos.startup:before", () => {})`',
      )),
      flatMap(() => toObservable(this._dataSource.onListen())),
    ));

    this.chaos.on("guildCreate:before", (guild) => of('').pipe(
      filter(() => this._dataSource.onJoinGuild),
      tap(() => this.logger.warn(
        'Your data source uses the deprecated onJoinGuild hook. ' +
        'Please check for an update, or let the developer know about the ' +
        'deprecation. The recommended replacement is ' +
        '`chaos.on("guildCreate:before", () => {})`',
      )),
      flatMap(() => toObservable(this._dataSource.onJoinGuild(guild))),
    ));
  }
