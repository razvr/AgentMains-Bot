...
class CommandManager {
  ...
  enableCommand(context, commandName) {
    ...
    return context.nix.dataManager
      .getGuildData(context.guild.id, 'core.enabledCommands')
      .do((commands) => commands[commandName] = true)
      .flatMap((commands) => context.nix.dataManager
        .setGuildData(context.guild.id, 'core.enabledCommands', commands))
  ...
  isCommandEnabled(context, commandName) {
    ...
    return Rx.Observable
      ...
      .flatMap(() => context.nix.dataManager.getGuildData(context.guild.id, 'core.enabledCommands'))
      .map((commands) => commands[commandName])
      .map((isEnabled) => {
