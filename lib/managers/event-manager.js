...
const { tap, flatMap, map, toArray, defaultIfEmpty, catchError } = require('rxjs/operators');
...
class EventManager extends ChaosManager {
  ...
  addEventListener(event, handler) {
    this.logger.warn("addEventListener is deprecated. Please use on instead.");
  ...
  triggerEvent(event, payload) {
    this.logger.warn("emit is deprecated. Please use emit instead.");
  ...
  emit(event, payload) {
    ...
    const runHandlers = (handlers) => {
      return flatMap(() => from(handlers).pipe(
        map((handler) => asPromise(handler(payload))),
        toArray(),
    ...
    of('').pipe(
      ...
      catchError((error) => of('').pipe(flatMap(async () => {
