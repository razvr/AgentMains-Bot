const { from, zip, of, fromEvent } = require('rxjs');
const { tap, flatMap, map, toArray, defaultIfEmpty, share } = require('rxjs/operators');
...
class EventManager {
  ...
  onListen() {
    Object.values(Discord.Constants.Events).forEach((event) => {
      fromEvent(
        ...
        flatMap((payload) => this.triggerEvent(event, payload)),
      ).subscribe();
    });
  }
  ...
  triggerEvent(event, payload) {
    ...
    of('').pipe(
      ...
      tap(() => this.chaos.logger.debug(`event ${event} complete`)),
    ).subscribe(
      () => doneSubject.next(''),
