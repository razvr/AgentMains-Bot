...
function reconnect(msgForSpy, overrideFlag) {
  if (!overrideFlag && reconnecting) { return }
  reconnecting = true;

  bot.login(cfg.token)
    .then(() => {
      console.log('Reconnected to discord');
      //messageSpy('I was disconnected from Discord. ' + msgForSpy);
      reconnecting = false;
    })
    .catch(() => {
      console.log('Failed to connect... retrying in 30 seconds');
      setTimeout(() => {
        reconnect(msgForSpy, true);
      }, 30000);
    });
}
...
bot.on('disconnect', (erMsg) => {
  ...
  let dontReconnectCodes = [
    4005,
  ];

  if (!dontReconnectCodes.includes(erMsg.code)) {
    reconnect('Error code was: ' + erMsg.code);
  }
});
