...
const { MockClient, MockMessage } = require("../test/mocks/discord.mocks");

describe('Response', function () {
  beforeEach(function () {
    this.discord = new MockClient();
    this.message = new MockMessage(this.discord);
  ...
  describe('#send', function () {
    ...
    context('when response type is "reply"', function () {
      ...
      it("returns an observable of the response from discord", async function () {
        let response = await this.response.send();
    ...
    context('when an unknown response type is used', function () {
      ...
      it("returns an observable that throws an error", async function () {
        try {
          ...
          expect(error.message).to.eq('Unknown response type unknown');
          return;
        }
